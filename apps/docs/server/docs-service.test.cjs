...
test('default content path follows the apps/docs/content layout', () => {
  assert.equal(
    resolveDocsDir({ env: {}, backendDir: path.join('workspace', 'apps', 'docs', 'server') }),
    path.resolve('workspace', 'apps', 'docs', 'content')
  );
});
