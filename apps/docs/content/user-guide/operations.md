...
1. Start the system using the appropriate command from the deployment guide.
2. Log in at `http://localhost` as `admin` with `AUTH_BOOTSTRAP_ADMIN_PASSWORD`.
...
4. If this is the first production run, verify `AUTH_BOOTSTRAP_ADMIN_PASSWORD`
   was set before the Auth database initialized.
...
| DefectDojo Products | `http://localhost/defectdojo/#products` |
