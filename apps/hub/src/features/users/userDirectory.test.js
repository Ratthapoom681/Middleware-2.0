...
test('the user directory exposes exactly the requested columns in order', () => {
  assert.deepEqual(
    ...
    ['Username', 'Full Name', 'Email', 'Company', 'Department', 'Role'],
  );
});
...
test('user search text contains only visible directory fields', () => {
  ...
  for (const visibleValue of ['analyst', 'test analyst', 'analyst@example.test', 'beenets', 'soc', 'viewer']) {
    assert.equal(searchText.includes(visibleValue), true);
  }
  for (const hiddenValue of ['000042', 'hidden-suspended', 'hidden-enabled', 'hidden-last-login']) {
