...
> [!CAUTION]
> The default `admin` / `admin` credentials are created automatically on first start. Change this password **immediately** — especially if the system is accessible over a network.
...
|---|---|---|
| Cannot reach `http://localhost:8080` | Gateway not running or wrong port | Run `docker compose ps` and check `GATEWAY_PORT` in `.env` |
...
| Resource | URL |
|---|---|
| Hub (Login & Switcher) | `http://localhost:8080/` |
...
| In-App Documentation | `http://localhost:8080/#docs` |
