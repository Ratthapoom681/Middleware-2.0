 ...
 *
 * Run while Hub and DefectDojo are listening.
 *   node smoke-test.cjs
 */
...
const AUTH_BASE = process.env.SMOKE_AUTH_BASE || 'http://localhost:3000';
const API_PREFIX = process.env.SMOKE_API_PREFIX || '/api';
const AUTH_PREFIX = process.env.SMOKE_AUTH_PREFIX || '/api';
let token = null;
let passed = 0;
let failed = 0;
const failures = [];
...
// ─── helpers ────────────────────────────────────────────────────────
const applyPrefix = (path, prefix) => (
    path.startsWith('/api') ? `${prefix}${path.slice('/api'.length)}` : path
);

const request = (method, path, body = null, headers = {}, base = BASE, prefix = API_PREFIX) =>
    new Promise((resolve, reject) => {
        const url = new URL(applyPrefix(path, prefix), base);
        const options = {
            method,
...
async function run() {
    ...
    console.log('\n2. Authentication');
    {
        const res = await request('POST', '/api/login', { username: 'admin', password: 'admin' }, {}, AUTH_BASE, AUTH_PREFIX);
    ...
    // 2b. Bad login
    {
        const res = await request('POST', '/api/login', { username: 'admin', password: 'wrong' }, {}, AUTH_BASE, AUTH_PREFIX);
    ...
    console.log('\n5. User management');
    {
        const res = await request('GET', '/api/users', null, authHeaders(), AUTH_BASE, AUTH_PREFIX);
    ...
    // 5b. Create a test user
    {
        const res = await request('POST', '/api/users', {
            ...
            status: 'active'
        }, authHeaders(), AUTH_BASE, AUTH_PREFIX);
    ...
    let smokeToken = '';
    {
        const res = await request('POST', '/api/login', { username: 'smoke_tester', password: 'test123' }, {}, AUTH_BASE, AUTH_PREFIX);
    ...
    {
        const onlineRes = await request('GET', '/api/users', null, authHeaders(), AUTH_BASE, AUTH_PREFIX);
        ...
        const logoutRes = await request('POST', '/api/logout', null, { Authorization: `Bearer ${smokeToken}` }, AUTH_BASE, AUTH_PREFIX);
        assert('  smoke_tester can logout', logoutRes.status === 200);

        const offlineRes = await request('GET', '/api/users', null, authHeaders(), AUTH_BASE, AUTH_PREFIX);
    ...
    // 5f. Suspend and reactivate test user
    {
        ...
        const blockedLogin = await request('POST', '/api/login', { username: 'smoke_tester', password: 'test123' }, {}, AUTH_BASE, AUTH_PREFIX);
        ...
        const relogin = await request('POST', '/api/login', { username: 'smoke_tester', password: 'test123' }, {}, AUTH_BASE, AUTH_PREFIX);
        assert('  reactivated smoke_tester can login', relogin.status === 200, `got ${relogin.status}`);
    }

    // 5g. Delete test user
    {
        const res = await request('DELETE', '/api/users/smoke_tester', null, authHeaders(), AUTH_BASE, AUTH_PREFIX);
    ...
    console.log('\n9. Dashboard sync SSE');
    {
        const sseOk = await new Promise((resolve) => {
            const url = new URL(applyPrefix('/api/sync/events', API_PREFIX), BASE);
            const options = {
                hostname: url.hostname,
                port: url.port,
                path: url.pathname,
    ...
    console.log('\n11. Logout');
    {
        const res = await request('POST', '/api/logout', null, authHeaders(), AUTH_BASE, AUTH_PREFIX);
