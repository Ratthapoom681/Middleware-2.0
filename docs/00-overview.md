...
| `users` | `#users` | Yes | Admin interface for creating and managing users |
...
The User Management screen (`UserManagement.jsx`) allows admins to manage user accounts and access levels.

### Key Features
- **User Directory Grid**: Lists usernames, roles, and allowed product scopes.
- **Create/Edit User Dialog**: Sets passwords, roles (`admin` or `viewer`), and product restrictions.
- **Access Scope Constraints**:
  - If a user is restricted to specific products, the frontend limits their findings and dashboard views to those products.
  - The backend also enforces these restrictions by filtering database queries.
